...
y = 1_600_000_000 * exp(x / a)

# 条件 1：x = 0 时，y = 1.5e9
# condition1 = Eq(y.subs(x, 0), 1_400_000_000)

# 条件 2：x = 7e26 时，y = 2e10
condition2 = Eq(y.subs(x, 6.5 * 10**26), 16_000_000_000)
...
def y_function(x):
    return a * b / 1e18 * math.exp((8 * 10**26 - x) / b) - a * b / 1e18
...
x_values = np.linspace(0, 8 * 10**26, 500)

# Generate y values
y_values = []
for c in x_values:
    y_values.append(y_function(c))
    

# Plot the function
plt.figure(figsize=(12, 6))
plt.plot(x_values, y_values, label=r'$y = 1.6 \times 3.474355855 \times 10^{17} \times e^{\frac{8 \times 10^{26} - x}{3.474355855 \times 10^{26}}} - 1.6 \times 3.474355855 \times 10^{17}$', color='b')
plt.xlabel('x')
plt.ylabel('y')
plt.grid(True)
plt.legend()
